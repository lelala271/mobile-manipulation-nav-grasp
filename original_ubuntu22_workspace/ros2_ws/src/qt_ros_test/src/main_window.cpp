...
void MainWindow::btn_disableall(bool _enabled)
{
    ...
    ui.pushButton_kcf->setDisabled(_enabled);
    ui.pushButton_orb->setDisabled(_enabled);
    ui.pushButton_rrt->setDisabled(_enabled);
    ui.pushButton_path1->setDisabled(_enabled);
    ui.pushButton_path2->setDisabled(_enabled);
    ui.pushButton_tts->setDisabled(_enabled);
    ui.pushButton_mic->setDisabled(_enabled);
    ui.pushButton_gu1->setDisabled(_enabled);
    ui.pushButton_gu2->setDisabled(_enabled);
    ui.pushButton_gu3->setDisabled(_enabled);
    ui.pushButton_ar1->setDisabled(_enabled);
    ui.pushButton_ar2->setDisabled(_enabled);
}
//关闭roslaunch进程下的所有功能
...
void qt_ros_test::MainWindow::on_checkBox_key_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("ros2 launch turn_on_wheeltec_robot wheeltec_sensors.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
...
//        QString strCmd = QString("ros2 launch wheeltec_nav2 save_map.launch.py\n");
//        emit sigSend(strCmd);
...
{
    QString str_cmd=QString("cd ~/wheeltec_ros2;colcon build --symlink-install --packages-select ")+ui.lineEdit_pkg->text()+"\n";
	ui.textEdit_cmd->append(str_cmd);
    emit sigSend(str_cmd);
}
...
void qt_ros_test::MainWindow::on_pushButton_kcf_clicked(bool checked)
{
    if(ui.pushButton_kcf->isChecked())
    {
        //QString strCmd = QString("ros2 launch wheeltec_robot_kcf wheeltec_robot_kcf.launch.py\n");
        //emit sigSend(strCmd);
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
                    "source /opt/ros/humble/setup.bash\n"
                    "source wheeltec_ros2/install/setup.bash\n"
        ...
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
        ui.pushButton_kcf->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开kcf跟随-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------------------正在关闭kcf跟随，请稍候-------------------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_orb_clicked(bool checked)
{
    if(ui.pushButton_orb->isChecked())
    {
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
                    "source /opt/ros/humble/setup.bash\n"
                    "source wheeltec_ros2/install/setup.bash\n"
        ...
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
        ui.pushButton_orb->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开ORB建图-------------------------------------------");
        QEventLoop loop;
        //创建单次定时器,槽函数为事件循环的退出函数
        QTimer::singleShot(2000, &loop, SLOT(quit()));
        //事件循环开始执行,程序会卡在这里,直到定时时间到,本循环被退出
        loop.exec();
        ui.textEdit_cmd->append("open ORB");
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("rviz2");
    }
    else
    {
        ...
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
                    "killall -3 python\n");
        ui.textEdit_cmd->append("-------------------------------正在关闭ORB建图，请稍候------------------------------");
    }
}
...
void qt_ros_test::MainWindow::on_pushButton_rrt_clicked(bool checked)
{
    if(ui.pushButton_rrt->isChecked())
    {
        ...
        ui.textEdit_cmd->append("-------------------------------------------正在打开rrt建图-------------------------------------------");
        QEventLoop loop;
        //创建单次定时器,槽函数为事件循环的退出函数
        QTimer::singleShot(2000, &loop, SLOT(quit()));
        //事件循环开始执行,程序会卡在这里,直到定时时间到,本循环被退出
        loop.exec();
        ui.textEdit_cmd->append("open rrt");
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("rviz2");
    }
    else
    {
        ...
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("killall -2 rviz2");
        ui.textEdit_cmd->append("-------------------------------正在关闭rrt建图，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_path1_clicked(bool checked)
{
    if(ui.pushButton_path1->isChecked())
    {
        QString strCmd = QString("ros2 launch wheeltec_nav2 wheeltec_nav2.launch.py\n");
        emit sigSend(strCmd);
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
        ...
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
        ui.pushButton_path1->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开设计路径并保存功能-------------------------------------------");
        QEventLoop loop;
        //创建单次定时器,槽函数为事件循环的退出函数
        QTimer::singleShot(2000, &loop, SLOT(quit()));
        //事件循环开始执行,程序会卡在这里,直到定时时间到,本循环被退出
    ...
    else
    {
        ...
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("killall -2 rviz2");
        ui.textEdit_cmd->append("-------------------------------正在关闭设计路径并保存功能，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_path2_clicked(bool checked)
{
    if(ui.pushButton_path2->isChecked())
    {
        QString strCmd = QString("ros2 launch wheeltec_nav2 wheeltec_nav2.launch.py\n");
        emit sigSend(strCmd);
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
        ...
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
        ui.pushButton_path2->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开路径跟踪功能-------------------------------------------");
        QEventLoop loop;
        //创建单次定时器,槽函数为事件循环的退出函数
        QTimer::singleShot(2000, &loop, SLOT(quit()));
        //事件循环开始执行,程序会卡在这里,直到定时时间到,本循环被退出
    ...
    else
    {
        ...
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("killall -2 rviz2");
        ui.textEdit_cmd->append("-------------------------------正在关闭路径跟踪功能，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_tts_clicked(bool checked)
{
    if(ui.pushButton_tts->isChecked())
    {
        QString strCmd = QString("ros2 launch tts tts_make.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_tts->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开tts文本转语音-------------------------------------------");
     }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭tts文本转语音，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_mic_clicked(bool checked)
{
    if(ui.pushButton_mic->isChecked())
    {
        QString strCmd = QString("ros2 launch wheeltec_mic_ros2 mic_init.launch.py\n");
        emit sigSend(strCmd);
        _cmd->write("ssh -Y wheeltec@192.168.0.100\n"
                    "source /opt/ros/humble/setup.bash\n"
                    "source wheeltec_ros2/install/setup.bash\n"
        ...
        btn_disableall(true);
        ui.pushButton_mic->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开语音控制-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭语音控制，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_gu1_clicked(bool checked)
{
    if(ui.pushButton_gu1->isChecked())
    {
        QString strCmd = QString("ros2 launch bodyreader bodyinteraction.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_gu1->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开姿态控制-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭姿态控制，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_gu2_clicked(bool checked)
{
    if(ui.pushButton_gu2->isChecked())
    {
        QString strCmd = QString("ros2 launch bodyreader bodyfollow.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_gu2->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开骨架识别：骨架跟随-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭骨架识别：骨架跟随，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_gu3_clicked(bool checked)
{
    if(ui.pushButton_gu3->isChecked())
    {
        QString strCmd = QString("ros2 launch bodyreader final.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_gu3->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开骨架识别-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭骨架识别，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_ar1_clicked(bool checked)
{
    if(ui.pushButton_ar1->isChecked())
    {
        QString strCmd = QString("ros2 launch aruco_ros aruco_recognize.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_ar1->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开AR标签识别-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭AR标签识别，请稍候------------------------------");
    }
}

void qt_ros_test::MainWindow::on_pushButton_ar2_clicked(bool checked)
{
    if(ui.pushButton_ar2->isChecked())
    {
        QString strCmd = QString("ros2 launch simple_follower_ros2 aruco_follower.launch.py\n");
        emit sigSend(strCmd);
        btn_disableall(true);
        ui.pushButton_ar2->setEnabled(true);
        ui.textEdit_cmd->append("-------------------------------------------正在打开AR标签跟随-------------------------------------------");
    }
    else
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------正在关闭AR标签跟随，请稍候------------------------------");
    }
}
